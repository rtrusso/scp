...
long long int Eval_Cycle_Count = 0;
int Alloc_Count = 0;
...
static INLINE pointer get_cell_x(scheme *sc, pointer a, pointer b) {
  if (sc->free_cell != sc->NIL) {
    if (debug) { fprintf(stderr,"[DEBUG] get_cell_x fcells %d\n",sc->fcells); }
    pointer x = sc->free_cell;
    sc->free_cell = cdr(x);
    --sc->fcells;
    return (x);
  }
  if (debug) { fprintf(stderr,"[DEBUG] get_cell_x _get_cell %d\n",sc->fcells); }
...
static pointer _get_cell(scheme *sc, pointer a, pointer b) {
  ...
  if(sc->no_memory) {
       if (debug) { fprintf(stderr,"_get_cell no_memory\n"); }
    return sc->sink;
  }

  if (sc->free_cell == sc->NIL) {
       const int min_to_be_recovered = sc->last_cell_seg*8;
       if (debug) { fprintf(stderr,"_get_cell sc->NIL\n"); }
    gc(sc,a, b);
    if (debug) { fprintf(stderr,"_get_cell after gc\n"); }
...
{
          if (debug) { fprintf(stderr,"[DEBUG] push_recent_alloc get_cell_x\n"); }
  pointer holder = get_cell_x(sc, recent, extra);
       if (debug) { fprintf(stderr,"[DEBUG] push_recent_alloc set typeflag\n"); }
  typeflag(holder) = T_PAIR | T_IMMUTABLE;
       if (debug) { fprintf(stderr,"[DEBUG] push_recent_alloc set contents\n"); }
  car(holder) = recent;
  cdr(holder) = car(sc->sink);
       if (debug) { fprintf(stderr,"[DEBUG] push_recent_alloc set sink\n"); }
  car(sc->sink) = holder;
       if (debug) { fprintf(stderr,"[DEBUG] push_recent_alloc set sink complete\n"); }
}
...
{
     pointer cell;
     if (debug) { fprintf(stderr,"[DEBUG] get_cell_x\n"); }
  cell   = get_cell_x(sc, a, b);
  ...
  if (debug) { fprintf(stderr,"[DEBUG] set_flag\n"); }
  typeflag(cell) = T_PAIR;
    if (debug) { fprintf(stderr,"[DEBUG] set contents\n"); }
  car(cell) = a;
  cdr(cell) = b;
      if (debug) { fprintf(stderr,"[DEBUG] push_recent_alloc\n"); }
  push_recent_alloc(sc, cell, sc->NIL);
        if (debug) { fprintf(stderr,"[DEBUG] push_recent_alloc complete\n"); }
  return cell;
}
...
pointer _cons(scheme *sc, pointer a, pointer b, int immutable) {
     if (debug) { fprintf(stderr,"[DEBUG] _cons get_cell\n"); }
  pointer x = get_cell(sc,a, b);

  if (debug) { fprintf(stderr,"[DEBUG] _cons set flag\n"); }
  typeflag(x) = T_PAIR;
  if(immutable) {
    setimmutable(x);
  }
  if (debug) { fprintf(stderr,"[DEBUG] _cons set contents\n"); }
  car(x) = a;
  cdr(x) = b;
  return (x);
}
 ...
 */
static void mark(pointer a) {
     static pointer badpointer = (pointer)0x0063F380;
     int extra_debug = 0;
     pointer t, q, p;
     ...
     p = a;
E2:
     if (debug_mark) { fprintf(stderr,"[DEBUG] mark(%p)\n",(void*)p); }
     if (debug_mark) { fprintf(stderr,"[DEBUG] mark type = %d\n",typeflag(p)); }
     /* extra_debug = debug_mark && (badpointer == p); */
     if (debug_mark) { fprintf(stderr,"[DEBUG] badpointer=%d\n",extra_debug); }
     setmark(p);
     if(is_vector(p)) {
          if (extra_debug) { fprintf(stderr,"[DEBUG] mark is_vector\n"); }
          int i;
     ...
     if (is_atom(p)) {
          if (extra_debug) { fprintf(stderr,"[DEBUG] mark is_atom\n"); }
          goto E6;
     }
     /* E4: down car */
     if (extra_debug) { fprintf(stderr,"[DEBUG] mark car\n"); }
     q = car(p);
     if (extra_debug) { fprintf(stderr,"[DEBUG] mark car = [%p]\n", (void*)q); }
     if (q && !is_mark(q)) {
...
static void gc(scheme *sc, pointer a, pointer b) {
  ...
  if (debug) { fprintf(stderr,"[DEBUG] mark globals, registers\n"); }
  /* mark system globals */
  mark(sc->oblist);
  mark(sc->global_env);

  if (debug) { fprintf(stderr,"[DEBUG] mark registers\n"); }
  /* mark current registers */
  mark(sc->args);
  if (debug) { fprintf(stderr,"[DEBUG] sc->NIL [%p]\n", (void*)sc->NIL); }
  if (debug) { fprintf(stderr,"[DEBUG] sc->EOF_OBJ [%p]\n", (void*)sc->EOF_OBJ); }
  if (debug) { fprintf(stderr,"[DEBUG] mark envir [%p]\n", (void*)sc->envir); }
  if (debug) { fprintf(stderr,"[DEBUG] mark envir type [%d]\n", typeflag(sc->envir)); }
  /* if (debug) { debug_mark = 1; } */
  mark(sc->envir);
  if (debug) { fprintf(stderr,"[DEBUG] mark code\n"); }
  mark(sc->code);
  if (debug) { fprintf(stderr,"[DEBUG] dump_stack_mark\n"); }
  dump_stack_mark(sc);
  mark(sc->value);
  mark(sc->inport);
  mark(sc->save_inport);
  ...
  if (debug) { fprintf(stderr,"[DEBUG] mark sink\n"); }
  ...
  if (debug) { fprintf(stderr,"[DEBUG] sweep\n"); }
  /* garbage collect */
  clrmark(sc->NIL);
  sc->fcells = 0;
  sc->free_cell = sc->NIL;
  ...
  */
  ...
  if(++gc_count%1000==0||sc->fcells>500000) {
       fprintf(stdout,";; fcells=%ld gc_count=%d Eval_Cycle_Count=%lld\n", sc->fcells, gc_count, Eval_Cycle_Count);
  }
}
...
static void s_save(scheme *sc, enum scheme_opcodes op, pointer args, pointer code) {
     if (debug) { fprintf(stderr,"[DEBUG] s_save 1\n"); }
    sc->dump = cons(sc, sc->envir, cons(sc, (code), sc->dump));
    if (debug) { fprintf(stderr,"[DEBUG] s_save 2\n"); }
    /* crash */
    sc->dump = cons(sc, (args), sc->dump);
    if (debug) { fprintf(stderr,"[DEBUG] s_save 3\n"); }
    sc->dump = cons(sc, mk_integer(sc, (long)(op)), sc->dump);
    if (debug) { fprintf(stderr,"[DEBUG] s_save done\n"); }
}
...
static pointer opexe_0(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_REAL_EVAL:
#endif
          if (debug) { fprintf(stderr,"[DEBUG] op_real_eval code [%p]\n",(void*)sc->code); }
          if (debug) { fprintf(stderr,"[DEBUG] op_real_eval code type [%d]\n",typeflag(sc->code)); }
          if (is_symbol(sc->code)) {    /* symbol */
               ...
               if (is_syntax(x = car(sc->code))) {     /* SYNTAX */
                    ...
                    if (debug) { fprintf(stderr,"[DEBUG] evaluating application\n"); }
                    if (debug) { fprintf(stderr,"[DEBUG] sc->NIL=[%p]\n", (void*)sc->NIL); }
                    if (debug) { fprintf(stderr,"[DEBUG] typeflag(sc->NIL)=[%d]\n", typeflag(sc->NIL)); }
                    s_save(sc,OP_E0ARGS, sc->NIL, sc->code);
                    if (debug) { fprintf(stderr,"[DEBUG] s_save complete\n"); }
                    /* If no macros => s_save(sc,OP_E1ARGS, sc->NIL, cdr(sc->code));*/
                    sc->code = car(sc->code);
                    if (debug) { fprintf(stderr,"[DEBUG] new code [%p]\n", (void*)sc->code); }
                    if (debug) { fprintf(stderr,"[DEBUG] new code type [%d]\n", typeflag(sc->code)); }
                    s_goto(sc,OP_EVAL);
               }
          } else {
               s_return(sc,sc->code);
...
static pointer opexe_4(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_ERR0:  /* error */
          ...
          fprintf(stderr,"[DEBUG] Eval_Cycle_Count=%lld\n",Eval_Cycle_Count);
          sc->args = cdr(sc->args);
          s_goto(sc,OP_ERR1);
...
static void Eval_Cycle(scheme *sc, enum scheme_opcodes op) {
  ...
  for (;;) {
    ++Eval_Cycle_Count;
       /* if (Eval_Cycle_Count >= 201600) { */
       /*      debug = 1; */
       /* } */
       /* if (debug && (Eval_Cycle_Count % 1 == 0)) { */
       /*      fprintf(stderr,"[DEBUG] Eval_Cycle_Count=%lld\n",Eval_Cycle_Count); */
       /* } */
    ...
    if (pcd->name!=0) { /* if built-in function, check arguments */
      ...
      int ok=1;
      int n;
      if (debug) { fprintf(stderr,"[DEBUG] built-in [%s]\n",pcd->name); }
      n=list_length(sc,sc->args);
    ...
    if (debug) { fprintf(stderr,"[DEBUG] ok_to_freely_gc\n"); }
    ok_to_freely_gc(sc);
    if (debug) { fprintf(stderr,"[DEBUG] dispatch sc->op=%d\n",(int)sc->op); }
