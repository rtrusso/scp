...
(define (schemec-debug-console . args)
  (if *schemec-debug*
      (let loop ((first #t)
                 (items args))
        (if (null? items)
...
(define (compile-scheme-program! code-to-translate main-symbol)
  ...
  (let ((env (scheme-codegen-environment)))
    (let loop ((code code-to-translate)
               (body (empty-insn-seq)))
      (schemec-debug-console "compile-file loop"
                             (if (not (null? code))
                                 (car code)
                                 code))
      (if (null? code)
          ...
          (let* ((unused-1 (if *schemec-debug* (begin (display ";; compiling " *output*)
                 ...
                 (unused-2 (if *schemec-debug* (begin (display ";; current label token: " *output*)
                                                      ...
                                                      (newline *output*))))
                 (statement (car code))
                 (unused-3 (schemec-debug-console "before expanded-statement"))
                 (expanded-statement (car (scheme-expand-macros (list statement) (get-compile-env))))
                 (unused-4 (schemec-debug-console "after expanded-statement "))
                 (statement-without-defines (car (rewrite-defines (list expanded-statement))))
                 (unused-5 (schemec-debug-console "after rewrite-defines"))
...
(define (expand-rfilact exp env ctx)
  (schemec-debug-console "expand-rfilact" exp)
  (if (and (list? exp)
           (= 2 (length exp))
           (string? (car (cdr exp))))
      (let ((result (rfilact-impl (car (cdr exp)))))
        (schemec-debug-console "rfilact result" result)
        (list 'quote result))
...
(define (compile-need! file-name)
  (schemec-debug-console "compile-need!" file-name)
  (if (not (file-visited? file-name))
...
(define (expand-file! file-name)
  (schemec-debug-console "expand-file!" file-name)
    ...
    (if (and (fs-exists? syntax-input-file-fs)
        ...
        (let ((syntax-input-port (open-input-file syntax-input-file-name)))
          ...
          (let ((loaded-syntax (read-fully syntax-input-port)))
            (scheme-expand-macros loaded-syntax (get-compile-env)))
        ...
        (begin
          ...
          (let ((input-port (open-input-file input-file-name))
            ...
            (newline)
            (begin-accumulating-define-syntax-expressions!)
            (schemec-debug-console "calling scheme-expand-macros")
            (scheme-expand-macros (read-fully input-port) (get-compile-env))
            (schemec-debug-console "done scheme-expand-macros")
            (stop-accumulating-define-syntax-expressions!)
...
(define (compile-file! file-name)
  (schemec-debug-console "compile-file!" file-name)
  (let ((output-file-name (get-output-file-name file-name))
        (output-file-name-tmp (get-output-file-name-tmp file-name))
        (syntax-output-file-name (get-syntax-output-file-name file-name))
        (syntax-output-file-name-tmp (get-syntax-output-file-name-tmp file-name))
        )
    (schemec-debug-console "compile-file! files opened" file-name)
    ...
    (let* ((input-port (open-input-file file-name))
      ...
      (begin-accumulating-define-syntax-expressions!)
      (schemec-debug-console "before compile-scheme-program!" file-name)
      (compile-scheme-program! (read-fully input-port)
                               (get-file-name-need-symbol file-name))
      (schemec-debug-console "after compile-scheme-program!" file-name)
      (if (not *expand-only*)
      ...
      (let ((arg (car args))
        ...
        (cond ((and (not (null? rest))
              ...
              ((long-opt? "conspiracy" arg)
               (set! *conspiracy-mode* #t)
               (set! *emit-global-marks* #f)
               (schemec-debug-console "enabled conspiracy mode")
               (install-special-form 'need expand-need)
