...
(define *needc-version* "generated by needc 01-20-2019 0.3")
(define *modules* '())
(define *root* "./")
...
(define (get-obj-file-name path)
  (replace-extension (flatten-path-to-file-name path) ".obj"))
...
(define (generate-script-file module-name file-name output-port)
  ...
  (let ((port (open-input-file file-name)))
    ...
    (if (not *expand-mode*)
        (begin
          (emit-line "nasm -fwin32 " (env-var "OUTDIR") *dir-sep*
                     (get-asm-file-name file-name)
